...
int main() {
    ...
    try
    {
        ...
        Body::Rigid platformBody(MassProperties(1e-4, Vec3(0),ballMass*UnitInertia::cylinderAlongX(1e-1, 1e-1))); // (Iz, Iy) ?
        ...
        // Одно колесо
        MobilizedBody::Planar platformFree(matter.Ground(),Transform (Rotation(-Pi/2, XAxis), Vec3(0, 1, -4)), platformBody,Vec3(0));
//        v1
//        MobilizedBody::Slider sliderFree(platformFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)));  // ???
//        MobilizedBody::Pin cylinderFree1(sliderFree, Transform(Vec3(-0.5,0,-1)), cylinder_1_Body, Transform(Vec3(0)));
//        v2
//        MobilizedBody::Screw sliderFree(platformFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)), 0);  // ???
//        MobilizedBody::Pin cylinderFree1(sliderFree, Transform(Vec3(-0.5,0,-1)), cylinder_1_Body, Transform(Vec3(0)));
//        v3
//        MobilizedBody::Slider sliderFree(platformFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)));  // ???
//        MobilizedBody::Screw screwFree(sliderFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)), 0);  // ???
//        MobilizedBody::Pin cylinderFree1(sliderFree, Transform(Vec3(0.5,0,2)), cylinder_1_Body, Transform(Vec3(0)));
//        v3
        MobilizedBody::Slider sliderFree(platformFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)));  // ???
        MobilizedBody::Screw screwFree(sliderFree, Transform( Rotation(Pi/2, YAxis), Vec3(0, 0, -1)), pinBody, Transform (Vec3(0)), 0);  // ???
        MobilizedBody::Pin cylinderFree1(screwFree, Transform(Rotation(Pi/2, YAxis), Vec3(0.5,0,2)), cylinder_1_Body, Transform(Vec3(0)));


//        MobilizedBody::Planar removalFree(sliderFree, Transform(Rotation(-Pi/2, ZAxis), Vec3(0, 0, -1)), removalBody,Vec3(0));
//        MobilizedBody::Pin cylinderFree1(sliderFree, Transform(Vec3(-0.5,0,-1)), cylinder_1_Body, Transform(Vec3(0)));
//        MobilizedBody::Pin cylinderFree2(matter.Ground(), Transform(Rotation(Pi/2, YAxis), Vec3(+2,0,0)), cylinder_2_Body, Transform(Vec3(0)));
        ...
        // Постоянные моменты
        // Кручение платформы | ок
//        torqueConstantController = Force::MobilityConstantForce(forces, platformFree,MobilizerUIndex(0), 5);
        // Движение по прямой | ок
//        torqueConstantController = Force::MobilityConstantForce(forces, platformFree,MobilizerUIndex(1), 100);

        // Координаты
//        platformFree.setDefaultQ(Vec3(1, 1, 1))
        ...
        cout << "t=" << state.getTime()
             << " q=" << platformFree.getQAsVector(state)
             << " u=" << platformFree.getUAsVector(state)
             << endl;
        ...
        while (true)
        {
            ...
            cout << "t=" << localState.getTime()
                 << " q=" << platformFree.getQAsVector(localState)
                 << " u=" << platformFree.getUAsVector(localState)
                 << endl;
//            std::ofstream file;
//            file.open ("/Users/alexeyzhuchkov/CLionProjects/simbody/data/" + std::string(now_str) + ".txt", std::ios_base::app);
