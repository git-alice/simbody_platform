...
int main() {
    try
    {
        ...
        // 1 вар - еще одно тело
        // 2 вар - custom MobilizedBody
        // 3 вар - добавить тело Translation + Pin
        MobilizedBody::Pin Platform(matter.Ground(),Transform (Vec3(0)), PlatformBody,Vec3(0));

        MobilizedBody::Planar removalFree(Platform,Transform(Rotation(-Pi/2, XAxis), Vec3(0, -2.01, 0)), removalBody,Vec3(0));  // ???
...
//        Force::MobilityDiscreteForce torque(forces, cylinderFree2, MobilizerUIndex(0), 0);
        // Force::MobilityConstantForce(forces, cylinderFree2, MobilizerUIndex(0), 0.01);
          torqueDiscreteController = Force::MobilityDiscreteForce(forces, cylinderFree1, MobilizerUIndex(0), 0);
...
//                                                                MobilizerUIndex(0), 0);
        ...
        while (true)
        {
            ...
            cout << "t=" << localState.getTime()
                 ...
                 << " q_removal=" << removalFree.getQAsVector(localState)
                 << endl;

            torqueDiscreteController.setMobilityForce(localState, cos(localState.getTime()) * 1000);
